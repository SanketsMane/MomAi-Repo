  ...
  const handleFileDrop = (acceptedFiles: File[]) => {
    ...
    if (file) {
      ...
      if (file.size > MAX_FILE_SIZE) {
        alert(`File too large! Maximum size is ${MAX_FILE_SIZE / (1024 * 1024)}MB. Please:\n• Split into smaller files\n• Use plain text format\n• Copy-paste content directly into chat`);
        return;
      }
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      if (blob.size > MAX_FILE_SIZE) {
        alert(`File too large! Maximum size is ${MAX_FILE_SIZE / (1024 * 1024)}MB. For larger files, please:\n\n1. Split into smaller sections\n2. Convert to plain text (.txt)\n3. Use online PDF splitters\n4. Copy-paste text directly into chat`);
